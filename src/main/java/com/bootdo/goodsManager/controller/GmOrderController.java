...
public class GmOrderController {
	 ...
	 *  获取当前用户订单列表 晋升逻辑
	 */
	private R promotion(UserDO user){
		try {
			...
			if(userLevel==5){
				...
				Integer pastYear = month-2>0?year:year-1;
				String monthStart = pastYear + "" + pastTwoMonth + "" + "01 00:00:00";




				// 经销商和总经销商 统计上个月晋升 则每个月一号统计上月的订单userLevel==4||userLevel==3
